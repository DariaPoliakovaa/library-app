...
export class BookForm {
    constructor(private bookLibrary: Library<Book>, private onBookAdded: () => void) {}

    public render(container: HTMLElement): void {
        const card = document.createElement('div');
        card.className = 'card mb-4 shadow-sm';
        card.innerHTML = '<div class="card-body"><h5 class="card-title mb-3">Додати нову книгу</h5></div>';
        ...
        form.addEventListener('submit', (e) => {
            e.preventDefault();
            if (Validation.isRequired(titleInput.input.value) && Validation.isYearValid(yearInput.input.value)) {
                const newBook = new Book(Date.now().toString(), titleInput.input.value, authorInput.input.value, parseInt(yearInput.input.value, 10));
